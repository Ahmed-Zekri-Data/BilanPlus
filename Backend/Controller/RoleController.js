    ...
    const transformPermissionsArrayToObject = (permissionsArray) => {
      ...
      if (Array.isArray(permissionsArray)) {
        permissionsArray.forEach(perm => {
          const key = perm.split(':')[0]; // e.g., "users:view" -> "users"
          const mappedKey = {
            users: 'gestionUtilisateurs',
            roles: 'gestionRoles',
            clients: 'gestionClients',
            suppliers: 'gestionFournisseurs',
            invoices: 'gestionFactures',
            accounting: 'gestionComptabilite',
            reports: 'gestionBilans',
            declarations: 'gestionDeclarations',
            advancedReports: 'rapportsAvances',
            system: 'parametresSysteme'
          }[key];
          if (mappedKey) {
            permissionsObject[mappedKey] = true;
          }
        });
      }
